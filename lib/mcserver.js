...
MinecraftServer.prototype._getSyncFrequency = function(frequencyString) {
  function s2ms(s) { return Math.abs(s * 1000); }
  function m2ms(m) { return Math.abs(m * 60 * 1000); }
  function h2ms(h) { return Math.abs(h * 60 * 60 * 1000); }
...
MinecraftServer.prototype._clearListeners = function() {
  ...
  if (self._automatedSync) {
    clearInterval(self._automatedSync);
  }
}
...
MinecraftServer.prototype._startAutomatedSync = function() {
  var self = this;
  log('Automated sync scheduled every ' + colors.cyan(options.syncEvery));
  self._automatedSync = setInterval(function() {
